...
from scripts.knapsacky import knapsack
...
class SummarizerSettings(Enum):
    ...
    HIGH_IMPORTANCE_VAL = 20  # conditions and constraints
    MEDIUM_IMPORTANCE_VAL = 10
    LOW_IMPORTANCE_VAL = 5
    RNN_AMBIGUOUS_VAL = 5
    RNN_PREDATORY_VAL = 20
    RNN_IMPORTANT_VAL = 10
...
class Summarizer:
    ...
    def _calculate_word_values(self, ignore_stop_words=True):
        ...
        for word in self._words.keys():
            ...
            if self._words[word] < mean + std:
                ...
                self._word_values[word] = min(int(SummarizerSettings.STD_MULT.value * ((self._words[word] - mean) / std)), int(SummarizerSettings.STD_MULT.value * 6))
    ...
    def _calculate_sentence_values(self, value_q_e=True):
        ...
        for s in range(len(self.__sentences)):
            ...
            if (len(self.__sentences[s]) < SummarizerSettings.WORD_THRES.value and s_object.value == 0) or \
                    len(self.__sentences[s]) <= SummarizerSettings.WORD_THRES.value - 3:
            ...
            if "last updated" not in s_object.text.lower():
                s_object.value += SummarizerSettings.MEDIUM_IMPORTANCE_VAL.value * len(list(legal.dates.get_dates(s_object.text)))
            ...
            if any(word in s_object.text.lower() for word in buzz_words):
                s_object.value += SummarizerSettings.MEDIUM_IMPORTANCE_VAL.value
            ...
            for word in parse_words(self.__sentences[s].text, to_lower=True):
                if len(word) > SummarizerSettings.WORD_LEN_THRES.value:
    ...
    def get_optimal_subset(self, max_num_words, ret_as=None):
        ...
        opt_val, opt_subset = knapsack(weights, values, max_num_words, override_default_only=True)
        if ret_as == "str":
